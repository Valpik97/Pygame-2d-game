...
class Level():
    ...
    def create_map_hitbox(self):
        ...
        for layer in self.map:
            ...
            for tile in layer:
                if tile != '0':
                    rects_tile.append(pygame.Rect(int(x), int(y), self.tile_size[0], self.tile_size[1]))
                x += self.tile_size[0]
            y += self.tile_size[1]
...
class Player():
    ...
    def move(self):
        ...
        self.rect, self.collision_types, self.hit_list = move(self.rect, rects_tile, self.movement)

        self.vertical_momentum += strehgth_gravity * dt
    ...
    def draw(self):
        ...
        display.blit(pygame.transform.flip(current_image, self.flip, False),
                     (int(self.rect.x - 60 - scrolling[0]), int(self.rect.y - 40 - scrolling[1])))

    def change_level(self, new_level):
        # Обновляем и сохраняем файл
        save_data = get_saves()
        save_data[number_save] = new_level
        save_data_string = ''
        for i in save_data:
            save_data_string += i + ','
    ...
    def looking(self, mousepos):
        if mousepos[0] <= self.rect.centerx - scrolling[0]:
            self.flip = True
        else:
            self.flip = False
...
class Enemy():
    ...
    def move(self):
        ...
        self.rect, self.collision_types, self.hit_list = move(self.rect, rects_tile
    ...
    def attack(self):
        ...
        if math.sqrt(abs((self.rect.centerx - player.rect.centerx) ** 2 + (
            ...
            if self.shoot_timer >= 60:
                self.slopex = (player.rect.centerx - scrolling[0]) - (self.rect.centerx - scrolling[0] + 5)
                self.slopey = (player.rect.centery - scrolling[1]) - (self.rect.centery - scrolling[1] + 35)
...
class Gun():
    ...
    def __init__(self, image):
        self.image = image
        self.x = player.rect.centerx + x_offset - scrolling[0]
        self.y = player.rect.centery + y_offset - scrolling[1]
...
class Projectile():
    ...
    def update(self):
        self.trajectory()
        self.collision_check(self.rect, rects_tile)
    ...
    def draw(self):
        ...
        display.blit(self.image, (int(self.x - scrolling[0] - self.radius), int(self.y - scrolling[1] - self.radius)))
...
class Particle():
    ...
    def draw(self):
        pygame.draw.circle(display, self.color, (int(self.x - scrolling[0]), int(self.y - scrolling[1])), int(self.radius))
...
def draw():
    global out_fade, in_fade, alpha_fade
    display.fill((180, 235, 235))
    ...
    if player.level == 'Tutorial':
        display.blit(instruction_img, (380 - scrolling[0], 380 - scrolling[1]))
    ...
    if out_fade:
        ...
        fade_surface.set_alpha(alpha_fade)
...
while True:
                ...
                if event.button == 1:
                    ...
                    for i, button in enumerate(save_buttons):
                        if button.is_over():
                            number_save = i
                            save_data = get_saves()
                            player.change_level(save_data[number_save])
                            game_menu_load = False
                            running_game = True
                            select_sound.play()
                            play_bgmusic()
    ...
    if screen_win:
        win_menu_main_button = Button(560, 550, 800, 200, (75, 173, 89), "Return to Main Menu", (0, 0, 0),
                                      pixel_font_large)
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                if event.button == 1:
                    if win_menu_main_button.is_over():
                        screen_win = False
                        menu_main = True
                        select_sound.play()
                        play_menu_music()
    ...
    if running_game:
        ...
        scrolling[0] += int((player.rect.x - scrolling[0] - (WINDOW_SIZE[0] / 2 + player.width / 2)) / 18) * dt
        scrolling[1] += int((player.rect.y - scrolling[1] - (WINDOW_SIZE[1] / 2 + player.height / 2)) / 18) * dt
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                if event.button == 1 and len(bullets) <= 20:
                    mx, my = event.pos
                    slopex = mx - (player.rect.centerx - scrolling[0] + 5)
                    slopey = my - (player.rect.centery - scrolling[1] + 35)
